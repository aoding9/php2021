...
<body>
  ...
  <div class="page-container">
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
        <a href="javascript:;" onclick="member_add('添加试题','/admin/member/add','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加试题</a>
      ...
      <table class="table table-border table-bordered table-hover table-bg table-sort">
        ...
        <tbody>
          ...
          <tr class="text-c">
            ...
            <td>{{$i->id}}</td>
            <td>{{$i->question}}</td>
            <td>{{$i->paper->paper_name}}</td>
            <td>{{$i->score}}</td>
            <td><a href="javascript:;" onclick="showOptions('{{$i->options}}')">查看选项</a></td>
            <td>{{$i->answer}}</td>
            <td>{{$i->created_at}}</td>
  ...
  <script type="text/javascript">
    ...
    function showOptions(options) {
      var options = options.split('~').join('<br>');
      // for(options)
      layer.alert(options,{title:'选项'});
    }
